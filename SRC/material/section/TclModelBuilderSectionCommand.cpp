			    ...
			    TCL_Char **argv, TclModelBuilder *theTclModelBuilder)
{
    ...
    if (buildSection(interp, theTclModelBuilder, secTag, isTorsion, GJ, Ys, Zs) != TCL_OK) {          //Xinlong
   ...
   if (sectionRepres->getType() == SEC_TAG_FiberSection)
   {
	 ...
	 if (currentSectionIsND)
	   section = new NDFiberSection3d(secTag, numFibers, fiber);
           ...
           section = new FiberSection3d(secTag, numFibers, fiber, &theGJ, Ys, Zs);  //Xinlong
	 }
	 else
		 section = new FiberSection3d(secTag, numFibers, fiber, 0, Ys, Zs); //Xinlong
