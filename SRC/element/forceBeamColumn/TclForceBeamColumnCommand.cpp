				   ...
				   TclModelBuilder *theTclBuilder)
{
  ...
  if ((strcmp(argv[6],"Lobatto") != 0) &&
	...
	sections[i] = theSection;
	  //Xinlong
	  if (Tcl_GetDouble(interp, argv[argi], &Ys) != TCL_OK) {
		  opserr << "WARNING invalid Ys\n";
		  opserr << argv[1] << " element: " << eleTag << endln;
		  return TCL_ERROR;
	  }
	  argi++;
	  if (Tcl_GetDouble(interp, argv[argi], &Zs) != TCL_OK) {
	...
	theElement = new DispBeamColumn3d(eleTag, iNode, jNode, nIP, sections, *beamIntegr, *theTransf3d, Ys, Zs, mass, cMass); //Xinlong
      else if (strcmp(argv[1], "dispBeamColumnThermal") == 0)
