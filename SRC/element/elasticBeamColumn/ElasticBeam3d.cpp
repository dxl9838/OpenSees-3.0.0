...
void* OPS_ElasticBeam3d(void)
{
	...
	if(OPS_GetIntInput(&numData,&transfTag) < 0) return 0;
	numData = 4;
	if (OPS_GetDoubleInput(&numData, &data[6]) < 0) return 0;
	theTrans = OPS_getCrdTransf(transfTag);
	if(theTrans == 0) {
	    opserr<<"no CrdTransf is found\n";
	    return 0;
	...
	return new ElasticBeam3d(iData[0],data[0],data[1],data[2],data[3],data[4],
				 data[5],iData[1],iData[2],*theTrans,data[6],data[7],data[8],data[9],mass,cMass);  //Xinlong
    }
}
...
ElasticBeam3d::ElasticBeam3d()
  ...
  A(0.0), E(0.0), G(0.0), Jx(0.0), Iy(0.0), Iz(0.0), Ys(0.0), Zs(0.0), betaY(0.0), betaZ(0.0), rho(0.0), cMass(0),    //Xinlong
...
ElasticBeam3d::ElasticBeam3d(int tag, double a, double e, double g, 
			     ...
			     CrdTransf &coordTransf, double ys, double zs, double betay, double betaz, double r, int cm, int sectTag)    //Xinlong
  :Element(tag,ELE_TAG_ElasticBeam3d), 
  A(a), E(e), G(g), Jx(jx), Iy(iy), Iz(iz), Ys(ys), Zs(zs), betaY(betay), betaZ(betaz), rho(r), cMass(cm), sectionTag(sectTag),
  Q(12), q(6), connectedExternalNodes(2), theCoordTransf(0)
{
...
ElasticBeam3d::getTangentStiff(void)
{
  ...
  double yc = -Ys;                    //Xinlong: yc is the y coord of centroid w.r.t. shear center
  double zc = -Zs;                    //Xinlong: zc is the z coord of centroid w.r.t. shear center
  
  q(0) = EAoverL*v(0)+EAoverL*yc*v(1)-EAoverL*yc*v(2)-EAoverL*zc*v(3)+EAoverL*zc*v(4); //Xinlong: N
  q(1) = EAoverL*yc*v(0)+(EIzoverL4+EAoverL*yc*yc)*v(1) + (EIzoverL2-EAoverL*yc*yc)*v(2)-EAoverL*yc*zc*v(3)+EAoverL*yc*zc*v(4);  //Xinlong: MIz
  q(2) = -EAoverL*yc*v(0)+(EIzoverL2-EAoverL*yc*yc)*v(1) + (EIzoverL4+EAoverL*yc*yc)*v(2)+EAoverL*yc*zc*v(3)-EAoverL*yc*zc*v(4); //Xinlong: MJz
  q(3) = -EAoverL*zc*v(0)-EAoverL*yc*zc*v(1)+EAoverL*yc*zc*v(2)+(EIyoverL4+EAoverL*zc*zc)*v(3) + (EIyoverL2-EAoverL*zc*zc)*v(4); //Xinlong: MIy
  q(4) = EAoverL*zc*v(0)+EAoverL*yc*zc*v(1)-EAoverL*yc*zc*v(2)+(EIyoverL2-EAoverL*zc*zc)*v(3) + (EIyoverL4+EAoverL*zc*zc)*v(4);  //Xinlong: MJy
  double Pu = q(0);
  double Mz = q(1) - q(0)*yc;
  double My = q(3) + q(0)*zc;
  double GJ = G * Jx + Pu * ((Iz + Iy) / A + yc * yc + zc * zc) + betaZ * Mz + betaY * My;
  double GJoverL = GJ*oneOverL;         // GJ/L
  q(5) = GJoverL*v(5);
  ...
  kb(0,0) = EAoverL;
  kb(1,1) = kb(2,2) = EIzoverL4+EAoverL*yc*yc; //Xinlong
  kb(2,1) = kb(1,2) = EIzoverL2-EAoverL*yc*yc; //Xinlong
  kb(3,3) = kb(4,4) = EIyoverL4+EAoverL*zc*zc; //Xinlong
  kb(4,3) = kb(3,4) = EIyoverL2-EAoverL*zc*zc; //Xinlong
  kb(5,5) = GJoverL;
  kb(1, 0) = kb(0, 1) = EAoverL * yc; //Xinlong
  kb(2, 0) = kb(0, 2) = -EAoverL * yc; //Xinlong
  kb(3, 0) = kb(0, 3) = -EAoverL * zc; //Xinlong
  kb(4, 0) = kb(0, 4) = EAoverL * zc; //Xinlong
  kb(3, 1) = kb(1, 3) = kb(4, 2) = kb(2, 4) = -EAoverL * yc*zc; //Xinlong
  kb(3, 2) = kb(2, 3) = kb(4, 1) = kb(1, 4) = EAoverL * yc*zc; //Xinlong
...
ElasticBeam3d::getInitialStiff(void)
{
  ...
  double EIyoverL2 = 2.0*Iy*EoverL;		// 2EIy/L
  double EIyoverL4 = 2.0*EIyoverL2;		// 4EIy/L
  double yc = -Ys;                      //Xinlong: y coord of centroid w.r.t. shear center
  double zc = -Zs;                      //Xinlong: z coord of centroid w.r.t. shear center
...
ElasticBeam3d::getResistingForce()
{
  ...
  q(0) = EAoverL * v(0) + EAoverL * yc*v(1) - EAoverL * yc*v(2) - EAoverL * zc*v(3) + EAoverL * zc*v(4); //Xinlong
  q(1) = EAoverL * yc*v(0) + (EIzoverL4 + EAoverL * yc*yc)*v(1) + (EIzoverL2 - EAoverL * yc*yc)*v(2) - EAoverL * yc*zc*v(3) + EAoverL * yc*zc*v(4); //Xinlong
  q(2) = -EAoverL * yc*v(0) + (EIzoverL2 - EAoverL * yc*yc)*v(1) + (EIzoverL4 + EAoverL * yc*yc)*v(2) + EAoverL * yc*zc*v(3) - EAoverL * yc*zc*v(4); //Xinlong
  q(3) = -EAoverL * zc*v(0) - EAoverL * yc*zc*v(1) + EAoverL * yc*zc*v(2) + (EIyoverL4 + EAoverL * zc*zc)*v(3) + (EIyoverL2 - EAoverL * zc*zc)*v(4); //Xinlong
  q(4) = EAoverL * zc*v(0) + EAoverL * yc*zc*v(1) - EAoverL * yc*zc*v(2) + (EIyoverL2 - EAoverL * zc*zc)*v(3) + (EIyoverL4 + EAoverL * zc*zc)*v(4); //Xinlong
  double Pu = q(0);
  double Mz = q(1) - q(0)*yc;
  double My = q(3) + q(0)*zc;
  double GJ = G * Jx + Pu * ((Iz + Iy) / A + yc * yc + zc * zc) + betaZ * Mz + betaY * My;
