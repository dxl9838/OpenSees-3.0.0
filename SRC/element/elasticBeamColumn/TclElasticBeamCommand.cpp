			       ...
			       int eleArgStart)
{
  ...
  if (ndm == 2) {
    ...
    // get the id, end nodes, and section properties
    int beamId, iNode, jNode, transTag;
    double A,E,G,Jx,Iy,Iz,Ys,Zs,betaY,betaZ;  //Xinlong
    if (Tcl_GetInt(interp, argv[1+eleArgStart], &beamId) != TCL_OK) {
      opserr << "WARNING invalid beamId: " << argv[1+eleArgStart];
	...
	return TCL_ERROR;
	  ...
	  //Xinlong
	  if (Tcl_GetDouble(interp, argv[11 + eleArgStart], &Ys) != TCL_OK) {
		  opserr << "WARNING invalid Ys - elasticBeamColumn " << beamId;
		  opserr << " iNode jNode A E G Jx Iy Iz Ys Zs betaY betaZ\n";
		  return TCL_ERROR;
	  }
	  if (Tcl_GetDouble(interp, argv[12 + eleArgStart], &Zs) != TCL_OK) {
		  opserr << "WARNING invalid Zs - elasticBeamColumn " << beamId;
		  opserr << " iNode jNode A E G Jx Iy Iz Ys Zs betaY betaZ\n";
		  return TCL_ERROR;
	  }
	  if (Tcl_GetDouble(interp, argv[13 + eleArgStart], &betaY) != TCL_OK) {
		  opserr << "WARNING invalid betaY - elasticBeamColumn " << beamId;
		  opserr << " iNode jNode A E G Jx Iy Iz Ys Zs betaY betaZ\n";
		  return TCL_ERROR;
	  }
	  if (Tcl_GetDouble(interp, argv[14 + eleArgStart], &betaZ) != TCL_OK) {
		  opserr << "WARNING invalid betaZ - elasticBeamColumn " << beamId;
		  opserr << " iNode jNode A E G Jx Iy Iz Ys Zs betaY betaZ\n";
		  return TCL_ERROR;
	  }
	  //Xinlong
	...
	return TCL_ERROR;
      ...
      double mass = 0.0;
      int cMass = 0;
      int argi = 15+eleArgStart; //Xinlong
	...
	if (strcmp(argv[argi],"-mass") == 0) {
      ...
      // now create the beam and add it to the Domain
      theBeam = new ElasticBeam3d (beamId,A,E,G,Jx,Iy,Iz,iNode,jNode, *theTrans, Ys, Zs, betaY, betaZ, mass, cMass);
    }
